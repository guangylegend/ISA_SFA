function [] = extract_unsupervised_training_data_hw2_layer1(train_video_dir, result_filename, params)
...
X = sample_video_blks(train_video_dir, train_filenames, params,sp_size, tp_size, num_patches);
save(result_filename, 'X', 'sp_size', 'tp_size', '-v7.3');
end

function X = sample_video_blks(path, filenames,params,sp_size, tp_size, num_perclip)
num_clips = length(filenames);

X = cell(num_clips/params.merge_clips,tp_size);
for i=1:num_clips/params.merge_clips
    for j=1:tp_size
    X{i,j} = zeros(sp_size^2, num_perclip*params.merge_clips);
    end
end
...
for i = 1 : num_clips/params.merge_clips
    ...
    for m = 1 : params.merge_clips
        filename = [path, filenames{(i-1)*params.merge_clips+m}];
