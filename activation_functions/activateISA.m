function act_p = activateISA(input, network_isa)
% t1 = tic;
%{
if nargin<3
...
while(count~=size(input,2))
    ...
    act_w = activateNN(input(:, count+1:batchend), network_isa.W, single(0), @sq_vec);    
    
    act_p(:, count+1:batchend) = activateNN(act_w, network_isa.H, single(1e-4), @sqrt_vec);
